test_that("You cannot assign values to opensimplex_space objects with `$`", {
  expect_error({
...
test_that("You can print opensimplex_space objects", {
  expect_no_error({
    space <- opensimplex_space()
    tf <- tempfile()
    sink(tf)
...
test_that("Depth should also be specified when slice is specified", {
  expect_error({
    space <- opensimplex_noise("S", width = 2, height = 2, slice = 3)
  }, "'depth' should not be missing when 'slice' is specified")
})
