...
#define PSSZIE 2048
#define PSMASK 2047
#define NS2 0.05481866495625118
#define NS3 0.2781926117527186
#define NS4 0.11127401889945551

LatticePoint2D *_newSLatticePoint2D(int xsv, int ysv)
{
...
LatticePoint3D *_newSLatticePoint3D(int xrv, int yrv, int zrv, int lattice)
{
...
LatticePoint4D *_newSLatticePoint4D(int xsv, int ysv, int zsv, int wsv)
{
...
LatticePoint2D **_newSLatticePoint2DConstArray()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		plp2DArr[i * 4 + 0] = _newSLatticePoint2D(0, 0);
		plp2DArr[i * 4 + 1] = _newSLatticePoint2D(1, 1);
		plp2DArr[i * 4 + 2] = _newSLatticePoint2D(i1, j1);
		plp2DArr[i * 4 + 3] = _newSLatticePoint2D(i2, j2);
	}
	return plp2DArr;
}
...
LatticePoint3D **_newSLatticePoint3DConstArray()
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		LatticePoint3D *c0 = _newSLatticePoint3D(i1, j1, k1, 0);
		LatticePoint3D *c1 = _newSLatticePoint3D(i1 + i2, j1 + j2, k1 + k2, 1);

		// (1, 0, 0) vs (0, 1, 1) away from octant.
		LatticePoint3D *c2 = _newSLatticePoint3D(i1 ^ 1, j1, k1, 0);
		LatticePoint3D *c3 = _newSLatticePoint3D(i1, j1 ^ 1, k1 ^ 1, 0);

		// (1, 0, 0) vs (0, 1, 1) away from octant, on second half-lattice.
		LatticePoint3D *c4 = _newSLatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + k2, 1);
		LatticePoint3D *c5 = _newSLatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + (k2 ^ 1), 1);

		// (0, 1, 0) vs (1, 0, 1) away from octant.
		LatticePoint3D *c6 = _newSLatticePoint3D(i1, j1 ^ 1, k1, 0);
		LatticePoint3D *c7 = _newSLatticePoint3D(i1 ^ 1, j1, k1 ^ 1, 0);

		// (0, 1, 0) vs (1, 0, 1) away from octant, on second half-lattice.
		LatticePoint3D *c8 = _newSLatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + k2, 1);
		LatticePoint3D *c9 = _newSLatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + (k2 ^ 1), 1);

		// (0, 0, 1) vs (1, 1, 0) away from octant.
		LatticePoint3D *cA = _newSLatticePoint3D(i1, j1, k1 ^ 1, 0);
		LatticePoint3D *cB = _newSLatticePoint3D(i1 ^ 1, j1 ^ 1, k1, 0);

		// (0, 0, 1) vs (1, 1, 0) away from octant, on second half-lattice.
		LatticePoint3D *cC = _newSLatticePoint3D(i1 + i2, j1 + j2, k1 + (k2 ^ 1), 1);
		LatticePoint3D *cD = _newSLatticePoint3D(i1 + (i2 ^ 1), j1 + (j2 ^ 1), k1 + k2, 1);
...
vect *_newSLatticePoint4DConstArray(LatticePoint4D **latticePoints)
{
  ...
  for (int i = 0; i < 256; i++)
  {
    ...
    int cz = ((i >> 4) & 3) - 1;
    int cw = ((i >> 6) & 3) - 1;
    latticePoints[i] = _newSLatticePoint4D(cx, cy, cz, cw);
  }
  for (int i = 0; i < 256; i++)
  {
    vect v;
...
Grad2 *_newSGrad2ConstArray()
{
	...
	arr[i++] = _newGrad2(-0.38268343236509, 0.923879532511287);
	arr[i++] = _newGrad2(-0.130526192220052, 0.99144486137381);
	Grad2 *gradients2D = _newGrad2Arr(PSSZIE);
	for (int i = 0; i < 24; i++)
	{
		arr[i].dx /= NS2;
		arr[i].dy /= NS2;
	}
	for (int i = 0; i < PSSZIE; i++)
	{
...
Grad3 *_newSGrad3ConstArray()
{
	...
	arr[i++] = _newGrad3(3.0862664687972017, 1.1721513422464978, 0.0);
	arr[i++] = _newGrad3(1.1721513422464978, 3.0862664687972017, 0.0);
	Grad3 *gradients3D = _newGrad3Arr(PSSZIE);
	for (int i = 0; i < 48; i++)
	{
		arr[i].dx /= NS3;
		arr[i].dy /= NS3;
...
Grad4 *_newSGrad4ConstArray()
{
	...
	arr[i++] = _newGrad4(0.7821684431180708, 0.4321472685365301, 0.4321472685365301, -0.12128480194602098);
	arr[i++] = _newGrad4(0.753341017856078, 0.37968289875261624, 0.37968289875261624, 0.37968289875261624);
	Grad4 *gradients4D = _newGrad4Arr(PSSZIE);
	for (int i = 0; i < 160; i++)
	{
		arr[i].dx /= NS4;
		arr[i].dy /= NS4;
 ...
 */
double _noiseS2_Base(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double xs, double ys)
{
	double value = 0;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		int pxm = (xsb + c->xsv) & PSMASK, pym = (ysb + c->ysv) & PSMASK;
...
double noiseS2(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double x, double y)
{
	...
	return _noiseS2_Base(ose, osg, xs, ys);
}
...
double noiseS2_XBeforeY(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double x, double y)
{
	...
	return _noiseS2_Base(ose, osg, yy + xx, yy - xx);
}
 ...
 */
double _noiseS3_BCC(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double xr, double yr, double zr)
{
	...
	while (c != NULL)
	{
		...
		{
			int pxm = (xrb + c->xrv) & PSMASK, pym = (yrb + c->yrv) & PSMASK, pzm = (zrb + c->zrv) & PSMASK;
 ...
 * Use noiseS3_XYBeforeZ or noiseS3_XZBeforeY instead, wherever appropriate.
 */
double noiseS3_Classic(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double x, double y, double z)
{
	...
	return _noiseS3_BCC(ose, osg, xr, yr, zr);
}
 ...
 * If Y is vertical in world coordinates, call noiseS3_XYBeforeZ(x, z, Y) or use noiseS3_XZBeforeY.
 * If Z is vertical in world coordinates, call noiseS3_XYBeforeZ(x, y, Z).
 * For a time varied animation, call noiseS3_XYBeforeZ(x, y, T).
 */
 ...
 * If Y is vertical in world coordinates, call noiseS3_XZBeforeY(x, Y, z).
 * If Z is vertical in world coordinates, call noiseS3_XZBeforeY(x, Z, y) or use noiseS3_XYBeforeZ.
 * For a time varied animation, call noiseS3_XZBeforeY(x, T, y) or use noiseS3_XYBeforeZ.
 */
 ...
 */
double _noiseS4_Base(OpenSimplexEnv *ose, OpenSimplexGradients *osg, double xs, double ys, double zs, double ws)
{
	double value = 0;
	...
	for (int i = 0; i < ose->LOOKUP_4D[index].length; i++)
	{
		...
		if (attn > 0)
		{
			...
			int pxm = (xsb + c->xsv) & PSMASK, pym = (ysb + c->ysv) & PSMASK;
			int pzm = (zsb + c->zsv) & PSMASK, pwm = (wsb + c->wsv) & PSMASK;
...
/**
 * 4D SuperSimplex noise, with XYZ oriented like noiseS3_Classic,
...
OpenSimplexEnv *initOpenSimplexS()
{
  LatticePoint4D **latticePoints = (LatticePoint4D **)malloc(sizeof(LatticePoint4D *) * 256);
	...
	ose->GRADIENTS_2D = _newSGrad2ConstArray();
	ose->GRADIENTS_3D = _newSGrad3ConstArray();
	ose->GRADIENTS_4D = _newSGrad4ConstArray();
	ose->LOOKUP_2D = _newSLatticePoint2DConstArray();
	ose->LOOKUP_3D = _newSLatticePoint3DConstArray();
	ose->VERTICES_4D = latticePoints;
	ose->LOOKUP_4D = _newSLatticePoint4DConstArray(latticePoints);
	return ose;
}
...
OpenSimplexGradients *newOpenSimplexSGradients(OpenSimplexEnv *ose, int64_t seed)
{
	...
	osg->perm = _newShortArr(PSSZIE);
	osg->permGrad2 = _newGrad2Arr(PSSZIE);
	osg->permGrad3 = _newGrad3Arr(PSSZIE);
	osg->permGrad4 = _newGrad4Arr(PSSZIE);
	short *source = _newShortArr(PSSZIE);
	for (short i = 0; i < PSSZIE; i++)
		source[i] = i;
