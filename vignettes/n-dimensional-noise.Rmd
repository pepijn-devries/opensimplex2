---
title: "N-Dimensional Noise"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{N-Dimensional Noise}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
knitr::knit_hooks$set(fullbleed = function(before, options, envir) {
  if (before) {
    par(mar = c(0,0,0,0), oma = c(0,0,0,0))
  }
})
library(ragg)
library(gifski)
set.seed(0)
```

TODO

```{r setup, fig.width=10, fig.height=10, fig.dpi=10, out.width="200px", message=FALSE, fullbleed=TRUE}
library(opensimplex2)
image(opensimplex_noise("F", 100, 100, frequency = 2),
      axes = FALSE, ann = FALSE, xaxs = "i", yaxs = "i")
set.seed(0)
image(opensimplex_noise("S", 100, 100, frequency = 2),
      axes = FALSE, ann = FALSE, xaxs = "i", yaxs = "i")

```

## Infinite Loop

```{r infinite-loop, fig.width=10, fig.height=10, fig.dpi=10, out.width="200px", animation.hook='gifski', interval=0.05, message=FALSE, fullbleed=TRUE}
scale_factor <- 50
space        <- opensimplex_space("S", 4L)
coords       <- expand.grid(z = 1:100, t = 1:100, frame = 1:100)
coords$x     <- 100*sin((2*pi*coords$frame - 1)/100)
coords$y     <- 100*cos((2*pi*coords$frame - 1)/100)
coords$value <- space$sample(.1*coords$x/scale_factor,
                             .1*coords$y/scale_factor,
                             2*coords$z/scale_factor,
                             2*coords$t/scale_factor)

for (i in 1:100) {
  frame_data <- subset(coords, frame == i)
  mat <- matrix(frame_data$value, 100, 100)
  image(mat,
        axes = FALSE, ann = FALSE, xaxs = "i", yaxs = "i",
        zlim = c(-1, +1))
}
```
